...
export default function HomeView({
  ...
  // Keep props compatibility to avoid unused variable build errors
  void isGenerating;
  ...
  const selectCategory = (cat: string) => {
    ...
    if (cat === "English Speaking") {
      setInputText(
        "Draft an English practice speech on giving construct feedback in corporate environments without demotivating team members.",
      );
  ...
  return (
    <div className="space-y-16 animate-fadeIn">
      ...
      <div className="text-center max-w-4xl mx-auto space-y-6">
        ...
        <p
          className={`text-lg md:text-xl max-w-2xl mx-auto font-normal ${theme === "dark" ? "text-zinc-400" : "text-zinc-600"}`}
        >
          Write, generate with AI, or reuse your existing scripts to perfect
          your delivery in a distraction-free environment.
        </p>
          ...
          >
                  ...
                  <span className="text-xs font-semibold text-indigo-400 flex items-center space-x-1.5 animate-pulse">
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 8H12M4 9h8"
                      />
                    </svg>
                    <span>AI Generating Speech (Connecting to Supabase)...</span>
                  </span>
                </div>
                <div
                  className={`w-full rounded-full h-1.5 overflow-hidden border ${theme === "dark" ? "bg-zinc-950/50 border-zinc-800" : "bg-zinc-100 border-zinc-200"}`}
                >
                  <div
                    className="bg-gradient-to-r from-indigo-500 to-purple-500 h-1.5 rounded-full animate-pulse w-full"
                  ></div>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between border-t border-zinc-800/10 dark:border-zinc-800/50 pt-4 gap-4">
              <div className="text-xs text-zinc-400 flex items-center space-x-2">
                {inputText.trim() && (
                  <span>
                    {inputText.trim().split(/\s+/).length} words entered
                  </span>
                )}
              </div>
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <h3
                    className={`font-display font-bold text-lg leading-snug line-clamp-1 group-hover:text-indigo-500 transition-colors ${theme === "dark" ? "text-zinc-100" : "text-zinc-900"}`}
                  >
                    {script.title}
                  </h3>
                  <span
                    className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded ${theme === "dark" ? "bg-zinc-800 text-zinc-400 border border-zinc-700/50" : "bg-zinc-100 text-zinc-600 border border-zinc-200"}`}
                  >
                    {script.category}
                  </span>
                </div>
                <p
                  className={`text-sm leading-relaxed line-clamp-3 ${theme === "dark" ? "text-zinc-400" : "text-zinc-600"}`}
                >
                  {script.content}
                </p>
              </div>
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2">
                  <h3
                    className={`font-display font-bold text-base leading-snug group-hover:text-indigo-500 transition-colors ${
